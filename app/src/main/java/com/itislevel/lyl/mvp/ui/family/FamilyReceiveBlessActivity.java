...
public class FamilyReceiveBlessActivity extends RootActivity<FamilyPresenter>
    ...
    private FamilyReceiveYuAdapter adapter;
    private int load_more = 0;
    private String feteid="";
    private Bundle bundle ;
    @Override
    protected int getLayoutId() {
        return R.layout.activity_family_receive_bless;
    }

    @Override
    protected void initEventAndData() {
        setToolbarTvTitle("收到的祭语");
    ...
    private void initAdapter() {
        if (adapter == null) {
            adapter = new FamilyReceiveYuAdapter(R.layout.item_family_yu_receive);
    ...
    public void familyBlessList(FamilyBlessListBean familyBlessListBean) {
        totalPage = familyBlessListBean.getTotalPage();
        load_more++;
        if(load_more==1)
        {
        ...
        //返回的数据 第一次
        if (pageIndex == 1 && adapter.getData().size() <= 0 && (familyBlessListBean.getList() == null
                || familyBlessListBean.getList().size() <= 0)) {
            View emptyView = View.inflate(this, R.layout.partial_empty_view, null);
            TextView viewById = emptyView.findViewById(R.id.tv_message);
            adapter.setEmptyView(emptyView);
        }
        if (isRefreshing) {
            adapter.setNewData(familyBlessListBean.getList());
            refreshLayout.setRefreshing(false);
        } else {
            adapter.addData(familyBlessListBean.getList());
