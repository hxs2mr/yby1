...
import com.itislevel.lyl.adapter.SpecialOrderReturnAdapter;
 ...
 */
public class OrderRefundFragment extends RootFragment<SpecialPresenter>
    ...
    private SpecialOrderReturnAdapter adapter;
...
//        request.put("status",Constants.SPECIAL_ORDER_REFUNDED+"");
        request.put("reftype", "203");

        mPresenter.specialReturnList(GsonUtil.obj2JSON(request));
    ...
    private void initAdapter() {
        if (adapter == null) {
            adapter = new SpecialOrderReturnAdapter(R.layout.item_order_refund);
...
//                    .build());
    ...
    public void specialReturnList(SpecialReturnBean returnBean) {
        totalPage = returnBean.getTotalPage();

        if (isRefreshing) {
            if (returnBean.getList()!=null){
                adapter.setNewData(returnBean.getList());
            }else{
            ...
            if (returnBean.getList()!=null)
                adapter.addData(returnBean.getList());
            refreshLayout.finishLoadmore();

        }
        //返回的数据 第一次
        if (pageIndex==1&&adapter.getData().size() <= 0 && (returnBean.getList()==null||returnBean.getList().size() <= 0)) {
            View emptyView = View.inflate(getActivity(), R.layout.partial_empty_view, null);
...
//        SpecialOrderDetailBean.ListBean item = this.adapter.getItem(position);
        // TODO: 2018-01-25 这里可能有问题 
        SpecialReturnBean.ListBean item = this.adapter.getItem(position);
        ...
        switch (view.getId()) {
            ...
            case R.id.tv_look_detail://查看详情
                ...
                ActivityUtil.getInstance().openActivity(getActivity(), OrderRefundDetailActivity
                        .class, bundle);
