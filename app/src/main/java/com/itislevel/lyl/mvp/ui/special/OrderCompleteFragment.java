 ...
 */
public class OrderCompleteFragment extends RootFragment<SpecialPresenter>
...
//        loadData();
    ...
    private void loadData() {
        ...
        request.put("status",Constants.SPECIAL_ORDER_RECEIVED+"");
    ...
    private void initAdapter() {
        if (adapter == null) {
            adapter = new SpecialOrderOtherAdapter(R.layout.item_order_complete);
...
//        totalPage = detailBean.getTotalPage();
        ...
        //返回的数据 第一次
        if (pageIndex==1&&adapter.getData().size() <= 0 && (detailBean.getList()==null||detailBean.getList().size() <= 0)) {
            View emptyView = View.inflate(getActivity(), R.layout.partial_empty_view, null);
