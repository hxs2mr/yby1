...
import com.itislevel.lyl.adapter.FamilyLetterAdapter;
import com.itislevel.lyl.adapter.FamilyReceiveYuAdapter;
...
public class FamilyLetterActivity extends RootActivity<FamilyPresenter>
        implements FamilyContract.View, BaseQuickAdapter.OnItemClickListener
        , BaseQuickAdapter.OnItemChildClickListener , SwipeRefreshLayout.OnRefreshListener,BaseQuickAdapter.RequestLoadMoreListener, View.OnClickListener{
    ...
    protected void initEventAndData() {
        ...
        write_letter.setOnClickListener(this);
    ...
    private void initAdapter() {
        if (adapter == null) {
            adapter = new FamilyLetterAdapter(R.layout.item_family_letter);
    ...
    private void loadData() {
        ...
        mPresenter.selectletter(GsonUtil.obj2JSON(request));
    }
    ...
    public void selectletter(LetterBean letterBean) {
        totalPage = letterBean.getTotalPage();
        load_more++;
        if(load_more==1)
        {
...
//返回的数据 第一次
        if (pageIndex == 1 && adapter.getData().size() <= 0 && (letterBean.getList() ==
                null || letterBean.getList().size() <= 0)) {
            View emptyView = View.inflate(this, R.layout.partial_empty_view, null);
            TextView viewById = emptyView.findViewById(R.id.tv_message);
            adapter.setEmptyView(emptyView);
        }
        if (isRefreshing) {
            adapter.setNewData(letterBean.getList());
            refreshLayout.setRefreshing(false);
        } else {
            adapter.addData(letterBean.getList());
    ...
    public void onItemClick(BaseQuickAdapter adapter, View view, int position) {
        ...
        bundle.putString("type","1");//0表示发布   1表示查看
        bundle.putString("title",  this.adapter.getData().get(position).getTitle());
        bundle.putString("fetename",  this.adapter.getData().get(position).getFetename()+"");
        bundle.putString("respectfully",  this.adapter.getData().get(position).getRespectfully());
        bundle.putString("comment",  this.adapter.getData().get(position).getComment());
        bundle.putString("createdtime",  timeSpanToDate(this.adapter.getData().get(position).getCreatedtime()));
        bundle.putString("looknum",this.adapter.getData().get(position).getLooknum()+"");
        bundle.putString("letterid",this.adapter.getData().get(position).getLetterid()+"");

        ActivityUtil.getInstance().openActivity(FamilyLetterActivity.this, FamilyLetterWriterActivity.class,bundle);
    }
    ...
    @Subscribe
    public void OnEvent(EventLetter eventLetter)
    {
        refreshLayout.setRefreshing(true);//刷新效果
        pageIndex = 1;
        isRefreshing = true;
    ...
    public void onClick(View view) {
        switch (view.getId())
        {
             case R.id.write_letter:
                 bundle.putString("feteid",feteid);
                 bundle.putString("type","0");//0表示发布   1表示查看
                 ActivityUtil.getInstance().openActivity(FamilyLetterActivity.this, FamilyLetterWriterActivity.class,bundle);
                break;
        }
    }
