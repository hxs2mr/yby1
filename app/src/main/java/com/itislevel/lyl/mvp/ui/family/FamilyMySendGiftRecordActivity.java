 ...
 * 祭祀记录
 */
public class FamilyMySendGiftRecordActivity extends RootActivity<FamilyPresenter>
    ...
    private FamilySendGiftAdapter adapter;
    private int load_more = 0;
    @Override
    protected int getLayoutId() {
        return R.layout.activity_family_my_record;
    }

    @Override
    protected void initEventAndData() {

        setToolbarTvTitle("祭祀记录");
        initAdapter();
    ...
    private void initAdapter() {
        if (adapter == null) {
            adapter = new FamilySendGiftAdapter(R.layout.item_family_gift_send);
    ...
    private void loadData() {
        ...
        mPresenter.familySendGift(GsonUtil.obj2JSON(request));
    ...
    public void familySendGift(FamilySendGiftRecordBean familySendGiftRecordBean) {
        totalPage = familySendGiftRecordBean.getTotalPage();
        load_more++;
        if(load_more==1)
        {
        ...
        //返回的数据 第一次
        if (pageIndex == 1 && adapter.getData().size() <= 0 && (familySendGiftRecordBean.getList() ==
                null || familySendGiftRecordBean.getList().size() <= 0)) {
        ...
        if (isRefreshing) {
            ...
            adapter.addData(familySendGiftRecordBean.getList());
