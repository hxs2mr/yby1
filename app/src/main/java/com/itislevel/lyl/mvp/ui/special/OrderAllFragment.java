...
import com.itislevel.lyl.adapter.SpecialOrderAllAdapter;
...
import com.itislevel.lyl.mvp.ui.main.customer.CustomerContract;
...
import com.itislevel.lyl.mvp.ui.main.customer.CustomerPresenter;
 ...
 */
public class OrderAllFragment extends RootFragment<SpecialPresenter>
    ...
    private SpecialOrderAllAdapter adapter;
...
//        loadData();
    ...
    private void loadData() {
        ...
        request.put("status", Constants.SPECIAL_ORDER_ALL + "");
     ...
     * 初始化adapter
     */
    private void initAdapter(List<SpecialOrderDetailBean.ListBean> data) {
        if (adapter == null) {
            adapter = new SpecialOrderAllAdapter(data);
...
//        totalPage = detailBean.getTotalPage();
        if (isRefreshing) {
            if (detailBean.getList() != null) {
                adapter = null;
                if (adapter == null) {
                    initAdapter(detailBean.getList());
...
//        adapter.remove(cancelPos);
    ...
    private void confirmOrder(int position) {
        ...
        mPresenter.specialOrderConfirm(GsonUtil.obj2JSON(request));
