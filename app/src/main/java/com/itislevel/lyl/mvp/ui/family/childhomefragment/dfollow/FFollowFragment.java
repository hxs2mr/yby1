...
import com.itislevel.lyl.adapter.FamilyFollowListAdapter;
...
public class FFollowFragment extends RootCancleFragment<FFollowPresenter> implements FFollowContract.View,
    ...
    private FamilyFollowListAdapter adapter;
    ...
    private void initAdapter() {
        if (adapter == null) {
            adapter = new FamilyFollowListAdapter(R.layout.item_family);
    ...
    @Override
    public void firstPage(FamilyFollowListBean funsharingListBeans) {
...
//返回的数据 第一次
    ...
    public void onItemClick(BaseQuickAdapter adapter, View view, int position) {
        FamilyFollowListBean.ListBean item = this.adapter.getItem(position);
    ...
    @com.itislevel.lyl.utils.rxbus.annotation.Subscribe(tag = RxBus.TAG_UPDATE, thread = EventThread.MAIN_THREAD)
    public void rxrefresh(String msg) {
        if (msg.equals("refresh")) {
            Logger.i("开始刷新");
            ToastUtil.Success("祭祀成功");
            refreshLayout.setRefreshing(true);//刷新效果
            pageIndex = 1;
            isRefreshing = true;
            isSearch=false;
